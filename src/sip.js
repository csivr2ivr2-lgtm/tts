...
export function createSipController(options = {}) {
  ...
  let ua = null;
  let socket = null;
  let status = user && hasCredential && uri ? "idle" : "disabled";
  let connected = false;
  ...
  async function connect() {
    if (!user || !hasCredential || !uri) {
      touch("missing-config", "disabled", "Set SIP_USER and either SIP_HA1 or SIP_PASSWORD in Hostinger environment variables");
  ...
  async function disconnect() {
    ...
    touch("stopped", user && hasCredential && uri ? "idle" : "disabled");
    return info();
  }
